...
void newTrackedPersonsAndLaserScanReceived(const TrackedPersons::ConstPtr& trackedPersons, const sensor_msgs::LaserScan::ConstPtr& laserScan) {
    ...
    try {
        ...
        g_transformListener->waitForTransform(laserScan->header.frame_id, trackedPersons->header.frame_id, meanTime, ros::Duration(g_hasWaitedForTransform ? 0.1 : 3.0));
