...
{        
    ...
    foreach(InitiatorCandidate::Ptr candidate, m_candidates)
    {
        ...
        // For first observation there exist no extrapolation just Gating
        else if(candidate->observations.size() == 1){
            ...
            foreach(Observation::Ptr observation, newObs)
            {
                if(compareWithCandidate(candidate, observation))
                {
                    ...
                    candidate->state = boost::dynamic_pointer_cast<KalmanFilterState>(
                        m_kalmanFilter.initializeTrackState(candidate->observations.back()) );
...
bool LogicInitiator::compareWithCandidate(const InitiatorCandidate::Ptr candidate, const Observation::Ptr newObservation)
{
    ...
    case MAHALANOBIS:
        ...
        double vminX (m_velMin);
        double vminY (m_velMin);
        double vmaxX (m_velMax);
        double vmaxY (m_velMax);

        if(std::abs(candidate->state->x()(2))- m_velocityVariance > m_velMin && std::abs(candidate->state->x()(2))+ m_velocityVariance < m_velMax)
        {
            vminX = std::abs(candidate->state->x()(2)) - m_velocityVariance;
            vmaxX = std::abs(candidate->state->x()(2)) + m_velocityVariance;
        }
        if(std::abs(candidate->state->x()(3)) - m_velocityVariance > m_velMin && std::abs(candidate->state->x()(3)) + m_velocityVariance < m_velMax)
        {
...
{
    double distMin((m_velMin*deltaTime)*(m_velMin*deltaTime));
    double distMax((m_velMax*deltaTime)*(m_velMax*deltaTime)+ SYSTEMATIC_SCAN_ERROR);
